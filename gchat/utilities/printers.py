...
def print_help_message():
    ...
    print(f"{BOLD}GPT-CLI – Made by @petterobam{END}")
    print("Command-line tool message using ChatGPT.")
    print(f"\n\n{UNDERLINE}Usage account call:{END} $ gchat {CYAN}[question]{END}")
    print(f"\n\n{UNDERLINE}Usage api key call:{END} $ gchat -v3 {CYAN}[question]{END}")


def print_invalid_language_message():
    print(f"\n{RED}Sorry, gchat doesn't support this file type.\n{END}")
...
def prompt_user_for_credentials_v3():
    print(f"{BOLD}Please enter your OpenAI credentials.{END}\n")
...
class LoadingMessage:
    def __init__(self, timeout=0.1):
        ...
        # self.steps = ["⢿", "⣻", "⣽", "⣾", "⣷", "⣯", "⣟", "⡿"]
        self.steps = ['-', '/', '|', '\\']
        self.message = f"{BOLD}{CYAN}Asking ChatGPT to answer your question{END}"
        self.end = f"{BOLD}{CYAN}🤖 ChatGPT's Answer:{END}"
